 ...
 */
package com.nyshex.cs.security.authentication;
...
/**
 * <p>
 * This is a simple {@link org.picketlink.idm.credential.Token.Provider} that manages a specific token type. In this
 * case the type is {@link com.nyshex.cs.security.authentication.JWSToken}.
 * </p>
 *
 * @author Pedro Igor
 * @see com.nyshex.cs.security.authentication.JWSToken
 */
@Stateless
public class JWSTokenProvider implements Token.Provider<JWSToken> {
    ...
    @Override
    public JWSToken issue(final Account account) {
        final TokenCredentialStorage tokenCredentialStorage = getIdentityManager().retrieveCurrentCredential(account,
                TokenCredentialStorage.class);
        final JWSToken token;
        if (tokenCredentialStorage == null) {
            final JWSBuilder builder = new JWSBuilder();
            final byte[] privateKey = getPrivateKey();
    ...
    @Override
    public JWSToken renew(final Account account, final JWSToken renewToken) {
